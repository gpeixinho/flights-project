...
class AirportCodeScraper(WikipediaScraper):
    def __init__(self, letter: str):
        ...
        if len(letter) != 1:
            raise ValueError(
                "parameter 'letter' attribute should be just one character"
            )
        if not letter.upper().isalpha():
            raise ValueError("parameter 'letter' should be an alphanumeric character")
